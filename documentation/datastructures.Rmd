...
Data structure (`list`) as used by `GetCombatModifier()` (in Rules.R). Each leaf element **shall** 
...
* Opponent
  ...
  * Movement `.Movement =` `r vecprint(.Movement)`
  * Evasive movements `r vecprint(.EvasiveMovement)`
  * TargetDistance `EvasiveMovement =` `.TargetDistance = ` `r vecprint(.TargetDistance)`
* Environment
