...
Data structure (`list`) as used by `GetCombatModifier()` (in Rules.R).
...
* Hero
  ...
  * `Mounted` TRUE/FALSE
  * Movement
    * if `OnFoot` is `TRUE`: `.Movement = ` `r vecprint(.Movement)`
    * if `Mounted` is `TRUE`: `.MountedMovement = ` `r vecprint(.MountedMovement)`
* Opponent
  * WeaponsRange: either `.CloseCombatRange` or `.RangedCombatRange`
  * Size `.TargetSize = ` `r vecprint(.TargetSize)`
  * Movement `.Movement =` `r vecprint(.Movement)`
  * `Zigzagging` TRUE/FALSE
