...
source(file.path("D:/Programmierung/R/DSADiceApp.git", "src/rules.R"))
setwd(file.path(rprojroot::find_rstudio_root_file(), "documentation"))
```

# Enumerations

Enumerations are realised as named atomic vectors. Functions like `ifelse()` or `as.integer()` may cause problems because the strip the attributes from the variables. If avoided, the labels will be preserved even after an assigment like `Scimitar <- .WeaponType["Melee"]` `r Scimitar <- .WeaponType["Melee"]`. One can access the label with `names(Scimitar)` which returns "`r names(Scimitar)`".
